...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Paket Upacara</h1>
    </div>

    <div class="col-lg-8">
        <form method="POST" action="/dashboard/paket/{{ $paket->slug }}" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
                <label for="nama_paket" class="form-label">Nama Paket</label>
                <input type="text" class="form-control @error('nama_paket') is-invalid  @enderror" id="nama_paket" name="nama_paket"
                    required autofocus value="{{ old('nama_paket', $paket->nama_paket) }}">
                @error('nama_paket')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid  @enderror" id="slug" name="slug"
                    required autofocus value="{{ old('slug', $paket->slug) }}">
                @error('slug')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="usaha_id">
                    @foreach ($usahas as $usaha)
                        @if (old('usaha_id', $paket->usaha_id) == $usaha->id)
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="category_id">
                    @foreach ($categories as $category)
                        @if (old('category_id', $paket->categori_id) == $category->id)
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('harga') is-invalid  @enderror" id="harga"
                    name="harga" required value="{{ old('harga', $paket->harga) }}">
                @error('harga')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('deskripsi') is-invalid  @enderror" id="exampleFormControlTextarea3" rows="3"
                    placeholder="Deskripsi" required id="deskripsi" name="deskripsi">{{ old('deskripsi', $paket->deskripsi) }}</textarea>
                @error('deskripsi')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('includes') is-invalid  @enderror" id="exampleFormControlTextarea1" rows="3"
                    placeholder="Keterangan tambahan" required id="includes" name="includes">{{ old('deskripsi', $paket->includes) }}</textarea>
                @error('includes')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('notes') is-invalid  @enderror" id="exampleFormControlTextarea2" rows="3"
                    placeholder="Notes tambahan" required id="notes" name="notes">{{ old('deskripsi', $paket->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                @if ($paket->image)
                    <img src="{{ asset('storage/' . $paket->image) }}" class="img-preview img-fluid mb-3 col-sm-6 d-block ">
                @else
            ...
            <button type="submit" class="btn btn-success mb-4 ">Update paket</button>
            <a href="/dashboard/paket" class="btn btn-warning  mb-4">Batal</a>
        </form>
    </div>

    <script>
        const name = document.querySelector("#nama_paket");
